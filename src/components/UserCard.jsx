...
export const UserCard = ({user, isImplementor, size='xl', radius='sm', edit}) => {
    const {mark, name, surname, id, patronymic} = user;
    ...
    return (
        <Link to={`${PATH.PROFILE}${id}` + (edit ? '?edit=true' : '')}>
            <Group position='apart' gap={5} align='stretch'>
                <Avatar mr={10} size={size} radius={radius} src={patronymic}>{name[0] + surname[0]}</Avatar>
