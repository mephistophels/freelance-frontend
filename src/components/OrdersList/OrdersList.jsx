...
function TypedOrderCard({type, task}){
  ...
  switch (type) {
    ...
    case PATH.ORDERS_OF_CLIENT:
      switch (status) {
        case 'done': return <OrderCard {...task} navigateTo={`${PATH.CLIENT_CLOSE_ORDER_ID}${task.id}`} showStatus={true}/>
        case 'in progress': return <OrderCard {...task} navigateTo={`${PATH.CLIENT_REDACT_ORDER_ID}${task.id}`} showStatus={true}/>
        case 'created': return <OrderCard {...task} navigateTo={`${PATH.CLIENT_REDACT_ORDER_ID}${task.id}`}
          showStatus={true} 
          showRedactMyOrderLink={true}
          applicationLinkTo={`/client/order/${task.id}/responses`}
        />
      }
  }
}
